...
header = ['Tag_Tuple', 'Tag1_Count','Tag2_Count', 'P(Tag1)', 'P(Tag2)', 'P(Tag1 AND Tag2)', 'P(Tag1 OR Tag2)']
...
for tag in cooccurence_counter.keys():
    writer.writerow(
                 {header[0]: tag,
                  ...
                  header[3]: round(tag_counter[tag[0]]/tag_total, 6),
		  header[4]: round(tag_counter[tag[1]]/tag_total, 6),
		  header[5]: round(cooccurence_counter[tag]/tag_total, 6),
		  header[6]: round((tag_counter[tag[0]] + tag_counter[tag[1]])/tag_total, 6)
		}
    )
